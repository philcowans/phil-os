...
{
  monitor_write("Starting to initialize tasking\n");
...
void switch_task()
{
  ...
  //  monitor_write_hex(current_task);
  ...
  if (eip == 0x12345) {
    //    monitor_write("Finished task switch\n");
    return;
  }

    // No, we didn't switch tasks. Let's save some register values and switch.

  // monitor_write("Magic word is: ");
  ...
  // monitor_write("We're about to jump to: ");
  // monitor_write_hex(eip);
  // monitor_write("\n");

  // monitor_write("Base pointer is: ");
  // monitor_write_hex(ebp);
  // monitor_write("\n");
  ...
  asm volatile("         \ 
     ...
     sti;                 \ 
     jmp *%%ecx           "
	       : : "r"(eip), "r"(esp), "r"(ebp), "r"(current_directory->physicalAddr) : "ecx", "eax");
}
